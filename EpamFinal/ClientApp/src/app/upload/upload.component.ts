...
export class UploadComponent {
  ...
  userDetails;
  constructor(private service: FilesService, private userService: RegistrationService) {
  ...
  public upload(event) {
    if (event.target.files && event.target.files.length > 0) {
      ...
      this.service.uploadFile(file, this.isPrivate, this.userDetails.userName).subscribe(
        data => {
          if (data) {
            switch (data.type) {
